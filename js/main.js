...
// Called via onerror="imgFallback(this, 'img/example.jpg')" on <img> tags.
// Shows a labeled placeholder box until a real file exists at that path,
// then the real photo displays automatically — no HTML changes needed later.
function imgFallback(imgEl, label) {
  imgEl.onerror = null;
  const wrap = imgEl.closest('.img-frame');
  if (!wrap) return;
  ...
  wrap.innerHTML =
    '<div class="img-placeholder">' +
      '<svg width="34" height="34" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">' +
        ...
        '<circle cx="8" cy="10" r="1.8" stroke="currentColor" stroke-width="1.6"/>' +
        '<path d="M2 16l5-5 4 4 3-3 8 8" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/>' +
      '</svg>' +
      '<span>Add photo:<br>' + label + '</span>' +
    '</div>';
}
